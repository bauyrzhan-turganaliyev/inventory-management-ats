...
The Testcontainers Docker API version is configured in `src/test/resources/docker-java.properties` for compatibility with current Docker versions.
...
The database schema is created automatically by Hibernate. If the category table is empty, the application initializes the default categories `Electronics`, `Books`, and `Food`.